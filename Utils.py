...
async def launch_tmux_bot(): # helper func generated by ChatGPT
    session_name = "SlashDiscordMusicBot"
    cwd = os.getcwd() # current working directory

    # Step 0: Kill the tmux session if it already exists
    ...
    # Step 1: Create the tmux session in the desired directory
    await asyncio.create_subprocess_exec(
        'tmux', 'new-session', '-d', '-s', session_name, f'cd {cwd}'
    )

    # Step 2: Send the command to run your script
    await asyncio.create_subprocess_exec(
        'tmux', 'send-keys', '-t', session_name, 'python3 musS_D.py', 'Enter'
    )